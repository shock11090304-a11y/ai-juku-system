...
### 18.11 実機で見つかった手書きの退行 (2026-08-15・修正済み)

「書く → スクロール → 書く」で 2 本目がスクロール量ぶんずれて記録された。
原因はページ枠 (getBoundingClientRect) のキャッシュをスクロールで無効化して
いなかったこと (リサイズと DPR だけだった)。スクロールで全部捨てる + 線の
書き始めで必ず測り直す、の 2 段で修正。browser_smoke に回帰検査を追加し、
修正を戻すと差 0.143 (= 150px ÷ ページ高さ) で落ちることを確認。
**塾長が実機 (iPad) で「直っています」と確認済み (2026-08-15)。**

### 18.10 取り込みの進み (2026-08-14 夜時点・再開用)

| 冊子 | 状態 |
|------|------|
| 英語R | ✅ 取り込み済み・**公開中** (22 問・1 問目 3 で照合済み) |
| 国語 / 化学 / 物理 / 生物 / 世界史 / 日本史 | ⬜ 未取り込み (JSON は塾長の `~/Desktop/kyotsu_json` に出力済み) |
| 数学IA / IIB | 対象外 (紙で配布) |

残りの手順は 1 冊 2〜3 分 × 6: 登録画面で「新しい冊子を作る」→ JSON 読み込み →
1 問目照合 (国語 5 / 化学 2 / 物理 2 / 生物 2 / 世界史 2 / 日本史 1) → 保存 → 公開。
PDF は塾長の Mac の `lesson-prints/2026-06-03_共通テスト◯◯_マーク式_問題.pdf`。
最後に iPad の生徒画面で 1 冊通し (開く → マーク → 提出 → 結果) を確認して完了。
...
### 20.4 製品名の決定 (2026-08-15)

**神テスト (kamitest)** に確定 (塾長決定)。紙×神のダブルミーニング。
一般販売を見据えた独立ブランドで、Trillion の名は製品に付けない
(買うのは他塾・学校のため)。画面表記: 生徒画面「神テスト」/
...
### 22.6 テスト環境での通し確認の記録 (2026-08-16)

Stripe サンドボックス + 本番 Vercel/Supabase で塾長が実施し、全経路を確認済み:
申込 (signup.html) → テストカード決済 → アカウント自動作成 (17:38) → 1分以内にログイン可 /
account.html「無料体験中・期限 8/19 (=3日後)」/ ポータル解約 =「8/19 に終了」(期間末まで利用可) /
ダッシュボードから今すぐキャンセル → 行が「解約済み」+ ログイン遮断 (User is banned)。
途中で見つかった 2 穴 (upsert の email 欠落 400 / account.html の hidden 無効) は
修正済みで、どちらも check_signup.py が再発を検査する。
