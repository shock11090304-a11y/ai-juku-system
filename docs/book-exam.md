...
### 18.2 ★ 正解の持ち方が 3 通りに割れている

| 形 | 科目 | 正解の在り処 | 変換 |
|----|------|--------------|------|
| A | 英語R (22) / 国語 (24) / 物理 (20) | トップレベル `answer_key:` | そのまま並べる |
| B | 数学IA / IIB | 大問ごとの `answer_key:` (ア・イ 方式) | **取り込めない (18.3)** |
| C | 化学 / 生物 / 日本史 / 世界史 | `answer_key` 無し。解説本文に `問1 → 正解 ②` | 正規表現で抜く |

形 C は書式が科目ごとに違う。変換器は **設問ごとに自分の解説から**抜く。
文書全体を舐めて順番に当てると、誤答の説明の番号を拾って静かにずれる。
1 問でも抜けなければその冊子は変換しない。

### 18.3 ★ 数学 IA / IIB は今のアプリに入れられない (9 冊中 7 冊だけ変換する)

マーク欄の答えは 1 桁の数字 (ア=5 等)。ところが `validateQuestions` は
**記述の正解が数字だけなら弾く** (`exam-book-admin-model.mjs:143`)。実測:
...
これは「選択肢を取りこぼした設問」を捕まえるための砦で、紙教材 835 問の変換で
実際に効いている。2 冊のために外す価値はない。選択式にするのも不可
(PDF には「ア」と出ているのに画面には 1〜10 のピルが出て、0 のマークが表せない)。

→ **入れたいなら検証器の設計変更が要る。指示があるまでやらない。**
...
### 18.5 ★ この変換器は元データを見ないで書いた

想定した形は `scripts/book_exam/fixtures_kyotsu/*.yaml` に書き下してある。
**実物が違えば変換は落ちる** — 推測しないので黙って通ることはない。
そのときは `--probe` の出力を見て、見本とアダプタを直すこと。

`--selftest` が証明するのは「3 通りのアダプタが動く」ことだけで、
「実物が変換できる」ことではない。ゲート (`check_book_exam.py` の 11c) が毎回回す。

壊して落ちることは確認済み (2026-08-14):

| 壊した所 | 捕まえるか |
|----------|-----------|
| ① を数字と見なくする | ○ 起算が決められなくなって落ちる |
| 形 B を拒まず変換する | ○ 「砦が外れている」で落ちる |
| 解説から抜く正規表現を壊す | ○ 「正解を抜けなかった」で落ちる |
| 起算不明を 0 起算へ倒す | ○ 「decide_base が甘い」で落ちる |

### 18.6 起算 (0 起算 / 1 起算) の扱い

`convert_workbook.py` と同じ理屈に加えて、**①②③ 表記なら 1 起算で確定**する
(① は必ず 1 番目)。決められないときは **推測せず止まる**。9 冊しかないので
人が 1 問見れば済む。黙って倒すと、その冊子は全問 1 つずれた正解になり、
画面はどこも壊れず、生徒だけが不正解になる。指定は `--base one` / `--base zero`。

### 18.7 解説に「正解 ③」が残る件 (問題なし)

形 C の解説は先頭に `問1 → 正解 ③` を含んだまま入る。
