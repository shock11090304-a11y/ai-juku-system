...
# 追加の注文があれば --note に (改行で複数)
python3 scripts/book_exam/make_llm_prompt.py math --n 10 \
    --unit "数学I 2次関数の最大最小（定義域つき）" \
    --note "誤答は計算ミスから作る
定義域が動く設問を 2 問入れる"
...
### 指示の出し方 — 書き換えるのは 2 か所だけ

指示文の本体 (規則・スキーマ・見本) は**触らない**。人が決めるのは次の 2 つ。

| 何を | どこに | 書き方 |
|---|---|---|
| **単元・題材** | `--unit` | 教科書の単元名 + 出題の角度。**1 行**で |
| **追加の注文** | `--note` | 1 行 1 件。無ければ省略してよい |

`--unit` の粒度 (これくらいが当たる):

| | 例 | 結果 |
|---|---|---|
| 粗すぎ | `数学` | 何が来るか分からない。単元がばらける |
| ちょうどよい | `数学I 2次関数の最大最小（定義域つき）` | 論点が 1 つに絞れて、10 問ぶんの幅もある |
| 細かすぎ | `y = x^2 - 4x + 1 の最小値を求める問題` | 10 問が同じ問題の焼き直しになる |

`--note` に書くとよいもの (実際に効いたもの):

- `誤答は計算ミスから作る` — 「明らかに変な選択肢」を減らせる
- `定義域が動く設問を 2 問入れる` — 論点の配分を指定できる
- `図は入れない` / `図を 1 つ入れる`
- `中学範囲を超えない` — レベルの上限を切る
- `会話文は 300 字以上にする` (本番相当のとき)

★ ChatGPT 側では、指示文を**プロジェクトの指示として保存**しておけば毎回貼らずに
済む。そのときは会話の 1 通目に「単元は 〜、追加の注文は 〜」とだけ書く。
