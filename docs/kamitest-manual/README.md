...
## 8.5 ChatGPT に問題を書かせる

**できます。** ただし**LLM の出力をそのまま冊子にはしません**。
...
python3 scripts/book_exam/make_llm_prompt.py japanese --honban --n 8 --out prompt.txt

# ② 出てきた指示文を ChatGPT に丸ごと貼る → 返ってきた JSON を spec.json に保存

# ③ 冊子にする (検査を通ったものだけ PDF と取り込み JSON になる)
python3 scripts/book_exam/materials/build_from_spec.py spec.json

# ④ 落ちたら「貼り戻す文」が出るので ChatGPT に渡して直させ、③ に戻る
```

### 機械が落とすもの (LLM がよく間違えるところ)

| 落ちる形 | なぜ危ないか |
|---|---|
| 知らない項目 (`difficulty` など) を足した | でっち上げの設定が黙って無視されるのを防ぐ |
| `answer` を 0 起算で書いた | **その冊子を解いた生徒が全員 0 点**になる |
| 解説の見出しの欠け・順番違い | 教科ごとの契約 |
| 解説に LaTeX (`\frac` `$`) を書いた | 画面は素のテキスト。記号のまま出る |
| 誤答の節に正解を載せた / 誤答が欠けた | 解説が選択肢とずれている合図 |
| **`check` の計算と正解の選択肢が合わない** | ★ 数学は sympy で**独立に計算**して照合する。LLM の答えを信用しない |
| 選択肢どうしが同値 | 正解が 2 つある問題になる |
| 記述の正解が数字だけ | 取り込みが弾く形 |

★ **`check` は数学の要**。設問ごとに「正解を計算する sympy の式」を書かせ、
機械が計算した値と `choices_plain[answer-1]` が一致しなければ落とします。
使える名前は指示文に列挙してあり、それ以外は読めません
(`__import__` のような文字列は式の形の時点で弾かれます)。

★ **機械に見えないものは残ります。** 誤答が別の読み方で正解にならないか、
本文の事実が正しいか、統計が古くなっていないか — ここは**人が全問読む**しかない
(§7 の③層)。LLM に書かせても、この工程は減りません。

★ 経路そのものの検査は `scripts/book_exam/check_llm_pipeline.py`。
見本の spec (`scripts/book_exam/fixtures_llm/sample_math.json`) が通ること、
壊した spec がその理由で落ちること、**指示文が正典とずれていないこと**を見ます。
