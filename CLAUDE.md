...
- ★**headless Chrome の `--window-size` はブラウザUI分を含む** (Linux で実測 87px・Mac は 0)。
  ...
  ★**測れなかったときに 0 を返さない**。0 は Mac では正しい値なので、
  「測れなかった」と区別がつかなくなり、フッターの無い画像が実寸検査を通ってしまう。
- ★**はみ出しは縦と横で壊れ方が違う**。縦は版面の外に出ずフッターに重なるので、
  版面の下を覗く検査は常に緑になる。横は撮影窓を版面と同じ幅にすると**写らない**。
  `build_vol.overflow_px()` が両方を実測して刷る前に落とす。
- 刷ったものをデスクトップに置くには `build_vol.py <key> --desktop`
  (`~/Desktop/trillion-ig/<key>/`)。任意の場所は `--out DIR`。
  ★配布先へ**直接刷らない**こと。刷る場所は `out_vol/` のまま (検査が見に行く場所)。
  ★クラウドの Claude Code は塾長の端末のデスクトップには書けない。塾長が Mac で回すこと。
- 検査: `python3 scripts/run_all_gates.py instagram_carousel`
  (本体のゲート + **そのゲート自体の変異試験 40 件**)。
  ★変異試験は「鳴るべき見張りの文言」まで指定する。「違反が 1 件でも出れば合格」に
  すると別のガードの巻き添えで通り、本命を消しても緑になる (実際に 18 種中 9 種がそうだった)。
