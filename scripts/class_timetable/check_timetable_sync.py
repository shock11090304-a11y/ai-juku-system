...
同じ表を**手で写している**のが次の 4 か所で、ズレると別々の壊れ方をする:
  ...
  academy.html / enrollment.html
                      ...
                      ★enrollment.html は 2026-09-23 に廃止 (vercel.json で Netlify の入塾申込書へリダイレクト) だが
                        このゲートの照合元として残してある。削除するとゲートが落ちる。
                      ★実際に保護者が見る入塾申込書 (Desktop/🏫 運営・集客/入塾書類/index.html・Netlify) は
                        リポジトリ外でこのゲートの対象外。時間割を変えたらその表も手で直して Netlify に再デプロイすること。
