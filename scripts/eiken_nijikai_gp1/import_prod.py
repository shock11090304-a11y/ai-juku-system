#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""指定の payload JSON を本番 (/api/admin/exam-questions/import) へ投入。
実行: railway run -s ai-juku-api python3 scripts/eiken_nijikai_gp1/import_prod.py [payload_file]
  第1引数で投入ファイルを指定 (既定 payload.json)。例: interview_payload.json
...
FNAME = sys.argv[1] if len(sys.argv) > 1 else "payload.json"
payload = json.load(open(os.path.join(HERE, FNAME), encoding="utf-8"))
print(f"📤 投入ファイル: {FNAME} ({len(payload.get('questions', []))} 行)")
