# -*- coding: utf-8 -*-
"""
★このコピーは「中学生 夏休み特訓［5科総合］」が BASE_CSS・esc・genko・render などの
  **部品だけ**を借りるためのもの。組版は book.py 側が持っており、
  この下の build_book と PART1/2/3 レンダラ・理科の図版部品はこのディレクトリでは使わない。
...
.qarow .b { flex:0 0 152px; border-bottom:1.2px solid #94a3b8; height:14px; }
...
.genko i { display:block; height:26px; border-right:1px solid #94a3b8;
...
def build_book(M, POINTS, PART1, PART2, PART3, outdir=DESKTOP):
    # ★このコピーでは使わない関数だが、呼ばれたときに理科の名前でPDFを上書きしないよう
    #   META 駆動にしておく（他のコピーと同じ形）。
    base = f'トリリオン_{M.get("series", "中学理科_演習")}_{M["no"]}'
    q_pdf = os.path.join(outdir, f'{base}_問題編.pdf')
    a_pdf = os.path.join(outdir, f'{base}_解答解説編.pdf')
    fl = f'中学理科 演習 {M["subtitle"]}'   # subtitle に既に No.xx が入っている
