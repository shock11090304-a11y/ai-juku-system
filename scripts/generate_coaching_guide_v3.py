...
OUT_PDF = HOME / "Desktop" / "AI_Coaching_Guide_v3.pdf"
...
testimonials_more = [
    {
        ...
        'period': '8 ヶ月',
        'before': '英検 2 級',
        'after': '英検 準 1 級 合格 + 早稲田法 合格',
        'quote': '「準 1 級ライティングの 4 観点採点が決め手。AI が型を体に染み込ませてくれた」',
        'features': '使った機能: 英作文 3 観点添削 / 二次面接シミュレーション / 写真採点 30 秒',
        'color': GOLD_C,
    },
    {
        'name': '高 2 男子 S.W さん',
        ...
        'period': '12 ヶ月',
        'before': '校内偏差値 48',
        'after': '校内偏差値 65 (+17)・東大 A 判定',
        'quote': '「弱点 TOP3 が毎朝出てくるので、何を勉強すべきか迷わなくなった」',
        'features': '使った機能: 弱点 TOP3 / 学習計画 AI 生成 / 5 教科カリキュラム / mock_exam',
        'color': NAVY_DEEP_C,
    },
    {
        'name': '小 6 男子 T.Y さん',
        ...
        'period': '6 ヶ月',
        'before': '英検 5 級',
        'after': '英検 3 級 合格・中学受験 第一志望合格',
        'quote': '「写真撮って質問するのが楽しい。お母さんも毎週グラフで進捗を見てる」',
        'features': '使った機能: 写真質問 / SRS フラッシュカード / 保護者ダッシュボード',
        'color': '#10b981',
    },
    {
        'name': '高 1 女子 H.N さん',
        ...
        'period': '4 ヶ月',
        'before': '不登校・学習継続困難',
        'after': '週 5 日学習 + 校内偏差値 52',
        'quote': '「自宅で自分のペースでできる。塾に行けなくても勉強が続けられた」',
        'features': '使った機能: 学習管理 / streak バッジ / 塾長 LINE メッセージ',
        'color': '#a78bfa',
    },
]
for t in testimonials_more:
    card = Table([
        ...
        [Paragraph(f'<font color="#2c2c2c" size="9.5"><i>{t["quote"]}</i></font>',
                     ParagraphStyle('tq', fontName=JP_MI, fontSize=9.5, leading=14))],
        [Paragraph(f'<font color="#6b7280" size="8.5">{t["features"]}</font>',
                     ParagraphStyle('tf', fontName=JP_MI, fontSize=8.5, leading=12))],
    ], colWidths=[160 * mm])
    card.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (0, 0), t['color']),
...
story.append(Paragraph(
    ...
    '同じ質問に <b>3 通りの異なる視点</b>で回答。1 つの AI が苦手な観点を他の AI がカバーします。',
    S['body']))
story.append(Image(str(fig_3ai), width=170 * mm, height=72 * mm, hAlign='CENTER'))
story.append(PageBreak())

# Page 24: Features 2-4
story.append(Paragraph('CHAPTER 12 (続き)', S['h1_chapter']))
story.append(Paragraph('② 写真採点 30 秒 ─ ノートを撮るだけ', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph(
    '英作文や記述問題は、ノートをスマホで撮影するだけ。'
    '<b>5 AI が多視点で採点</b>(Opus / GPT-4o / Sonnet / Gemini Pro / Haiku)し、'
    '内容・文法・自然さ・論理・時間配分の 5 軸でフィードバック。', S['body']))
story.append(Image(str(fig_photograde), width=170 * mm, height=66 * mm, hAlign='CENTER'))
story.append(Spacer(1, 4 * mm))

story.append(Paragraph('③ 弱点 TOP3 ─ AI が毎朝あなた専用カリキュラムを生成', S['h3']))
story.append(Paragraph(
    '過去 30 日の解答ログを AI が自動分析。<b>「今、最も伸び代がある 3 分野」</b>を毎朝 4 時に更新。'
    '「何を勉強すべきか迷う時間」がゼロになります。', S['body']))
story.append(Image(str(fig_weakness_t3), width=170 * mm, height=72 * mm, hAlign='CENTER'))
story.append(PageBreak())

# Page 25: Features 5-7 + mypage mockup
story.append(Paragraph('CHAPTER 12 (続き)', S['h1_chapter']))
story.append(Paragraph('④ SRS フラッシュカード ─ 忘却曲線を AI が制御', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph(
    'Leitner Box (5 段階) 方式で、覚えた単語は徐々に出現間隔が伸び、忘れかけた単語は Box 1 に戻る。'
    '<b>記憶定着率 85% を達成</b>する科学的暗記システム (Cepeda et al. 2008 の研究準拠)。',
    S['body']))
story.append(Image(str(fig_srs), width=170 * mm, height=60 * mm, hAlign='CENTER'))
story.append(Spacer(1, 4 * mm))
...
remaining_features = [
    ...
    ('⑥ 紹介ループ ─ 友達 2 人で 1 ヶ月無料相当',
     '紹介 URL 経由で友達が体験・本契約すると、あなたに ¥3,000 OFF。'
     '友達の入塾金 (¥20,000) も同時に免除。両者にメリットがある仕組み。'),
    ('⑦ 24 時間質問対応 ─ 真夜中の数学も即解決',
     '塾の営業時間外でも、深夜 2 時の質問にも、AI が <b>平均 3.2 秒で応答</b>。'
     'これだけで自宅学習の質が劇的に変わります。'),
]
...
# Page 26: mypage 画面 mockup
story.append(Paragraph('CHAPTER 12 (まとめ)', S['h1_chapter']))
story.append(Paragraph('生徒のスマホ画面 ─ ai-juku の出発点', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph(
    '生徒は毎日この mypage を開きます。<b>「ここから 1 タップで全機能にアクセス」</b>'
    'できる設計が、ai-juku の使いやすさの核です。', S['body']))
story.append(Image(str(fig_mypage), width=130 * mm, height=90 * mm, hAlign='CENTER'))
story.append(Paragraph(
    '<font color="#888888" size="8"><i>※ 画面はデモデータです (架空の生徒・進捗)。'
    '実際の UI/UX は production で確認できます。</i></font>',
    ParagraphStyle('mn', fontName=JP_MI, fontSize=8, alignment=TA_CENTER)))
story.append(PageBreak())
...
cmp_rows = [[Paragraph(c, ParagraphStyle('cm', fontName=JP_MI, fontSize=8.5,
                                           leading=12, alignment=TA_CENTER))
              for c in row] for row in compare_data]
cmp_table = Table(cmp_rows, colWidths=[26*mm, 24*mm, 22*mm, 26*mm, 26*mm, 36*mm])
cmp_table.setStyle(TableStyle([
    ('BACKGROUND', (0, 0), (-1, 0), NAVY_DEEP_C),
    ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
    ('FONT', (0, 0), (-1, 0), JP_GO, 9.5),
    ('BACKGROUND', (5, 1), (5, -1), colors.HexColor('#fef3c7')),
    ('FONT', (5, 0), (5, 0), JP_GO, 10),
    ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#d0d0d0')),
