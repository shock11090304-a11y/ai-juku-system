...
def band(kind):
    return ('<div class="band"><div class="l">%s <small>%s</small></div>'
            '<div class="r">準1級 / Pre-1<br>%s</div></div>'
...
def render_writing(p4):
    h = ('<div class="partttl pb">大問4　英作文'
         '<small>要約問題（Summary）と意見論述（Opinion）</small></div>')
    # 要約
    h += ('<div class="instr">■ 要約問題：次の英文を読み、その内容を <b>60〜70語</b> の英語で要約しなさい。</div>')
    for i, t in enumerate(p4["summary_tasks"], 1):
        h += '<div class="q"><div class="h"><span class="qno">要約 %d</span></div>' % i
        h += '<div class="ptitle" style="margin-top:4px">%s</div>' % esc(t.get("source_title",""))
        # source_text を段落化
        st = esc(t["source_text"])
        paras = re.split(r"\n\s*\n", st.strip())
        body = "".join('<p>%s</p>' % p.strip().replace("\n"," ") for p in paras if p.strip())
        h += '<div class="srcbox">%s</div>' % body
        h += '<div class="wlbox">' + '<div class="wl"></div>'*7 + '</div>'
        h += '<div class="wcnt">語数（　　　　語）　※60〜70語</div></div>'
    # 意見論述
    h += ('<div class="instr" style="margin-top:12px">■ 意見論述：以下の TOPIC について、あなたの意見とその理由を <b>120〜150語</b> の英語で書きなさい。'
          'POINTS のうち2つを使うこと。</div>')
    for i, t in enumerate(p4["essay_tasks"], 1):
        ...
        h += '<div class="points">' + "".join('<span class="point">%s</span>' % esc(p) for p in t["points"]) + '</div>'
        h += '<div class="wlbox">' + '<div class="wl"></div>'*13 + '</div>'
        h += '<div class="wcnt">語数（　　　　語）　※120〜150語</div></div>'
    return h
