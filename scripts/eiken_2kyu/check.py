...
if p2:
    ...
    for pa in ps:
        ...
        real = wc(pa["text"])
        if not (150 <= real <= 320): warn(f"{tag} 本文語数={real} が想定外(150-320)")
        for b in pa["blanks"]:
            btag = f"{tag}#{b.get('n')}"
...
if p3:
    ...
    for pa in ps:
        ...
        if not (200 <= real <= 420): warn(f"{tag} 本文語数={real} が想定外(200-420)")
...
if p4:
    ...
    for t in p4["summary_tasks"]:
        n = wc(t["model_summary"]); s = wc(t["source_text"])
        print(f"  {t['id']}: source={s}語, summary={n}語")
        if not (42 <= n <= 58): err(f"要約 {t['id']} model_summary={n}語 (45-55目安/42-58外)")
        if not (120 <= s <= 180): warn(f"要約 {t['id']} source={s}語 (150前後想定)")
    for t in p4["essay_tasks"]:
        n = wc(t["model_essay"])
        print(f"  {t['id']}: essay={n}語, points={len(t.get('points',[]))}")
        if not (75 <= n <= 110): err(f"意見 {t['id']} model_essay={n}語 (80-100目安/75-110外)")
        if len(t.get("points",[])) != 3: warn(f"意見 {t['id']} points={len(t.get('points',[]))} (3想定)")
