...
const SOURCE = 'kyotsu-eng2026-similar-20260624'
...
`第2問型(調査・意見)。本文は「運営/学校/団体が投稿した告知または調査結果」＋「2名の利用者/学生の名前付きコメント」。350〜450語。設問4問・各四択。必ず1問は「事実と意見の区別」(例: 〜の意見を最もよく表すのは/報告として事実なのは)。他は詳細理解・推論。コメント主の主張との一致/不一致を問う設問を含める。` },
  FMT3: { label:'第3問 物語＋順序', part_key:'r_long', numQ:3, minKeep:2, instr:
`第3問型(物語+順序)。一人称の短い体験談320〜400語。設問3問: (1)理由/きっかけを問う四択、(2)「出来事を起きた順に並べたものとして正しいのは」という"順序の組合せ"を1つ選ぶ設問(選択肢は「②→④→⑤→①」のような完全な並び順を4〜6個提示し、正しい並びは1つだけ・他は順序が崩れている)、(3)登場人物の心情/学び/形容詞を問う四択。物語内の出来事は明確な時系列を持たせる。` },
  FMT4: { label:'第4問 文章校正', part_key:'r_long', numQ:4, minKeep:3, instr:
`第4問型(文章校正)。本文は「生徒が書いた下書き(部誌/エッセイ/告知)」。本文中の下線部を (1) (2) (3) (4) のように番号付きの語句で示し、文の挿入候補位置を <A> <B> <C> <D> として本文テキスト中に文字で埋め込む。本文の後に「Comments:」として指導者の指摘((1)〜(4)に対応)を載せる。設問4問・各四択: (1)下線部の後に続けるのに最適な語句、(2)与えられた1文を入れるのに最適な位置(選択肢は「<A>」「<B>」「<C>」「<D>」の4つ)、(3)下線部の文を置き換えるのに最適な文、(4)主旨をまとめる下線部を置き換えるのに最適な語句。挿入位置や下線は必ず本文中に文字として存在させ、図に頼らない。` },
  FMT5: { label:'第5問 複数資料統合', part_key:'r_long', numQ:5, minKeep:3, instr:
`第5問型(複数資料統合)。本文は3資料を連結: ①チラシ/リーフレット(イベント告知・条件付き箇条書き)、②記入済みのフォーム/申込み(複数エントリ。項目例: 名前/年/価格/作者/コメント)、③主催者からのメール。450〜550語。設問5問・各四択: 目的、内容真偽、複数エントリを横断する「組合せで正しい記述」(選択肢は「AとB」式の組合せを1択で選ぶ・六択可)、含まれない/該当しないもの、メールが依頼する事柄(「AとC」式の組合せ1択)。資料間の照合で解ける設計にする。` },
  FMT6: { label:'第6問 物語＋アウトライン', part_key:'r_long', numQ:4, minKeep:3, instr:
`第6問型(物語+アウトライン整理)。本文は物語450〜550語＋「Story Outline」(登場人物の特徴・重要な出来事・主題を整理する枠をテキストで提示)。設問4問・各四択または六択: (1)ある登場人物を最もよく表す「2つの組合せ」を1択で選ぶ(選択肢は「①と③」式・六択)、(2)出来事を起きた順に並べた"順序の組合せ"を1択(完全な並びを提示)、(3)ある人物について本文が伝えることを1択、(4)主人公について本文が伝える主題を1択。` },
  FMT7: { label:'第7問 論説＋スライド', part_key:'r_long', numQ:5, minKeep:3, instr:
`第7問型(学術論説+スライド/要点)。本文は一般向けの説明的論説(心理・脳科学・社会・健康・環境などの話題)450〜550語＋発表用スライドの下書き(見出しと空所のある箇条書きをテキストで提示)。設問5問・各四択または六択: (1)現象が起こる条件/時を1択、(2)効果/利点の「2つの組合せ」を1択(六択)、(3)ある段階で起きることを1択、(4)研究知見の要約を1択、(5)複数の人物プロフィール(生活習慣)を本文中に提示し、本文の助言に最も合致する「2人の組合せ」を1択(六択)。専門用語は本文で平易に説明する。` },
  FMT8: { label:'第8問 意見統合＋資料', part_key:'r_long', numQ:5, minKeep:3, instr:
`第8問型(意見統合+資料)。本文は「あるテーマ(スポーツと技術/SNSと社会/AIと教育/環境と消費 等)についての小論文プロジェクト」。複数の名前付き意見(4〜5名)＋ Source A(短い記事)＋ Source B(調査データ)。Source B のグラフは描かず、数値を本文に表または箇条書きで inline する(例:「靴の快適さ: 一般ランナー74%, 学生選手57%, プロ60%」)。450〜650語。設問5問・各四択: ある人物の意見の要約、2人に共通する意見、ある立場を最も支持する人物(「AとB」式の組合せ1択可)、Source A に基づく根拠、Source B(inlineデータ)に基づき結論を支持する記述。データ参照問題は本文の数値だけで判定できるようにする。` },
}
