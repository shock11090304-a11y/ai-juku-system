...
// --- 講師用への入口 -----------------------------------------------------------
// ★ 配信の入口 (/) は受験画面に転送され、ホーム画面のアイコンもそこへ着く。
//   講師用の登録画面は別 URL なのに画面上に道が無く、URL を手打ちするしかなかった
//   (2026-08-17 塾長指摘)。講師のときだけ出す — 生徒に見せると迷いのもとになる。
ok('生徒には「講師用」ボタンを出さない',
  await page.evaluate(() => {
    const b = document.querySelector('#to-admin');
