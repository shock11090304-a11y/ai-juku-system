...
OUT_PDF = HOME / "Desktop" / "AI_Coaching_Guide_v5.pdf"
...
story.append(Paragraph('〜 35 機能 × 実画面で見る ai-juku の全て 〜', S['cover_sub']))
story.append(Spacer(1, 30 * mm))
...
story.append(Paragraph('Student Handbook Series Vol.3 — 2026 年 5 月 完全実画面版',
                        S['cover_brand_sub']))
...
# ============================================================
# CH 12 (page 23-24): ai-juku の最高の部分 7 機能
# ============================================================
story.append(Paragraph('CHAPTER 12', S['h1_chapter']))
story.append(Paragraph('ai-juku の最高の部分 ─ 他塾には絶対ない 7 機能', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 5 * mm))

story.append(Paragraph(
    'ここからは、<b>ai-juku がなぜ「24 時間動く家庭教師」と呼ばれるのか</b>を、'
    '7 つの代表機能で説明する。他塾の問題集・個別指導・他社 AI ではどれも実現していない、'
    'ai-juku 独自の組み合わせです。', S['body']))
story.append(Spacer(1, 4 * mm))

# ===== CH 12 mockup → 実スクショに差し替え (塾長指示) =====
def _real_screenshot(path_obj, max_width_mm=165, max_height_mm=170):
    """実スクショを navy 枠付きで埋め込み・縦横比保持 + max height clamp"""
    try:
        from PIL import Image as PILImage
        with PILImage.open(path_obj) as pim:
            pw, ph = pim.size
            calc_h = max_width_mm * ph / pw
            if calc_h > max_height_mm:
                w_mm = max_height_mm * pw / ph
                h_mm = max_height_mm
            else:
                w_mm = max_width_mm
                h_mm = calc_h
    except Exception:
        w_mm, h_mm = max_width_mm, max_width_mm * 0.7
    img = Image(str(path_obj), width=w_mm * mm, height=h_mm * mm)
    box = Table([[img]], colWidths=[w_mm * mm + 4])
    box.setStyle(TableStyle([
        ('BOX', (0, 0), (-1, -1), 1.2, NAVY_C),
    ...
    wrapper = Table([[box]], colWidths=[170 * mm])
...
# Feature 1: 3 AI 並列 — 実スクショ ai_tutor_photo (3 AI 選択 UI 表示)
story.append(Paragraph('① 3 AI 並列 ─ 世界 3 大 AI が同時に解答', S['h3']))
story.append(Paragraph(
    'Claude Opus 4.7 (Anthropic) + GPT-4o (OpenAI) + Gemini 2.5 Pro (Google) が、'
    '同じ質問に <b>3 通りの異なる視点</b>で回答。1 つの AI が苦手な観点を他の AI がカバー。'
    '解答モード「自動 / 通常 (3 AI) / 精密 (3 AI × 2 回 = 30 秒)」を選択できます。',
    S['body']))
story.append(Spacer(1, 3 * mm))
story.append(_real_screenshot(SCREENSHOT_DIR / 'ai_tutor_photo_1440x1400.png',
                              max_width_mm=160, max_height_mm=130))
story.append(Paragraph(
    '※ trillion-ai-juku.com/ai-tutor-photo.html の実画面',
    S['caption']))
story.append(PageBreak())

# Page 24: Feature 2 (写真採点) — モバイル実スクショ
story.append(Paragraph('CHAPTER 12 (続き)', S['h1_chapter']))
story.append(Paragraph('② 写真採点 30 秒 ─ スマホで撮るだけ', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph(
    'ノートや問題集を <b>スマホで撮影</b>するだけ。3 AI が並列で解答した後、'
    '記述式・国語問題は <b>5 AI 多視点採点</b> (Opus / GPT-4o / Sonnet / Gemini Pro / Haiku) で'
    '内容・文法・自然さ・論理・時間配分の 5 軸からフィードバック。', S['body']))
story.append(Spacer(1, 3 * mm))
# モバイル実画面 (塾長指示「生徒のスマホ画面も実際のスクショでいい」)
story.append(_real_screenshot(SCREENSHOT_MOBILE_DIR / 'ai_tutor_photo_mobile_390x1300.png',
                              max_width_mm=70, max_height_mm=170))
story.append(Paragraph(
    '※ 生徒の iPhone から見た実画面 (写真質問 UI)',
    S['caption']))
story.append(PageBreak())

# Page 25: Feature 3-4: 弱点 TOP3 + SRS
story.append(Paragraph('CHAPTER 12 (続き)', S['h1_chapter']))
story.append(Paragraph('③ 弱点 TOP3 + ④ SRS 単語帳', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph('③ 弱点 TOP3 ─ AI が毎朝あなた専用カリキュラム生成', S['h3']))
story.append(Paragraph(
    '過去 30 日の解答ログを AI が自動分析。<b>「今、最も伸び代がある 3 分野」</b>を毎朝 4 時に更新。'
    '「何を勉強すべきか迷う時間」がゼロになります。', S['body']))
story.append(Spacer(1, 2 * mm))
story.append(Paragraph('④ SRS フラッシュカード ─ 忘却曲線を AI が制御', S['h3']))
story.append(Paragraph(
    'Leitner Box (5 段階) 方式で覚えた単語は出現間隔が伸び、忘れかけた単語は Box 1 に戻る。'
    '<b>記憶定着率 85% を達成</b>する科学的暗記システム (Cepeda et al. 2008 準拠)。',
    S['body']))
story.append(Spacer(1, 3 * mm))
# 実スクショ: vocab_mobile (SRS フラッシュカード モバイル UI)
story.append(_real_screenshot(SCREENSHOT_MOBILE_DIR / 'vocab_mobile_390x1300.png',
                              max_width_mm=70, max_height_mm=150))
story.append(Paragraph(
    '※ AI 単語帳 SRS の実画面 (モバイル) ─ レベル選択 / 大学別頻出 / 音声読み上げ',
    S['caption']))
story.append(PageBreak())

# Page 26: Features 5-7 (テキストのみ・density up)
story.append(Paragraph('CHAPTER 12 (続き)', S['h1_chapter']))
story.append(Paragraph('⑤ 16,661 設問プール + ⑥ 紹介ループ + ⑦ 24h 対応', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

remaining_features = [
    ('⑤ 16,661 設問プール ─ 他塾の問題集 60 冊分',
     '英検・TOEFL・TOEIC・IELTS・大学入試・理系 を網羅した 162 part・16,661 問。'
     'すべて AI チームの 3 視点検閲 (内容・教育・入試) を通過した、再現可能な「型」問題。'
     '市販問題集を 60 冊買うのと同等の演習量を、月額 ¥14,500 で提供。'),
    ('⑥ 紹介ループ ─ 友達 2 人で 1 ヶ月無料相当',
     '紹介 URL 経由で友達が体験・本契約すると、紹介者は <b>月額 ¥3,000 OFF (永久)</b>。'
     '紹介された友達の入塾金 (¥20,000) も同時に免除。両者にメリットがある仕組み。'
     '友達 2 人紹介で ¥6,000/月 OFF = 通常月額の約 41% off に。'),
    ('⑦ 24 時間質問対応 ─ 真夜中の数学も即解決',
     '塾の営業時間外でも、深夜 2 時の質問にも、AI が <b>平均 3.2 秒で応答</b>。'
     'これだけで自宅学習の質が劇的に変わります。深夜の「分からない」を翌朝に持ち越さない、'
     '24 時間動く家庭教師として機能します。'),
]
for title, desc in remaining_features:
    story.append(Paragraph(title, S['h3']))
    story.append(Paragraph(desc, S['body']))
    story.append(Spacer(1, 3 * mm))
story.append(PageBreak())

# Page 27: 生徒のスマホ画面まとめ (実スクショ 3 つ並べ)
story.append(Paragraph('CHAPTER 12 (まとめ)', S['h1_chapter']))
story.append(Paragraph('生徒のスマホ画面 ─ 実画面ギャラリー', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 4 * mm))

story.append(Paragraph(
    '生徒は毎日 <b>iPhone でこの画面を開いて</b> 学習を開始します。'
    'モバイル最適化された UI で、片手で全機能にアクセスできる設計が ai-juku の使いやすさの核。'
    '以下は実際の trillion-ai-juku.com を iPhone viewport で撮影した生画面です。', S['body']))
story.append(Spacer(1, 4 * mm))

# 3 つのスマホ画面を横並びで表示
mobile_screens = [
    ('ai_tutor_photo_mobile_390x1300.png', '写真質問 UI'),
    ('vocab_mobile_390x1300.png', 'AI 単語帳'),
    ('mock_exam_mobile_390x1400.png', '完全模試'),
]
mobile_cells = []
mobile_captions = []
for fname, cap in mobile_screens:
    img_path = SCREENSHOT_MOBILE_DIR / fname
    if img_path.exists():
        try:
            from PIL import Image as PILImage
            with PILImage.open(img_path) as pim:
                pw, ph = pim.size
                h_mm = 50 * ph / pw  # 幅 50mm 固定
        except Exception:
            h_mm = 130
        if h_mm > 140:
            h_mm = 140
        cell_img = Image(str(img_path), width=50 * mm, height=h_mm * mm)
        cell_box = Table([[cell_img]], colWidths=[52 * mm])
        cell_box.setStyle(TableStyle([
            ('BOX', (0, 0), (-1, -1), 1, NAVY_C),
            ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#0a1f44')),
        ...
        mobile_cells.append(cell_box)
        mobile_captions.append(Paragraph(cap, ParagraphStyle('mc', fontName=JP_GO,
                                                              fontSize=9, alignment=TA_CENTER,
                                                              textColor=NAVY_DEEP_C)))

if len(mobile_cells) == 3:
    montage = Table([mobile_cells, mobile_captions],
                     colWidths=[55 * mm, 55 * mm, 55 * mm])
    montage.setStyle(TableStyle([
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ...
    story.append(montage)
    story.append(Spacer(1, 3 * mm))
    story.append(Paragraph(
        '※ すべて trillion-ai-juku.com で実際に撮影した本物の画面 '
        '(個人情報は一切含まれていません)',
        S['caption']))
story.append(PageBreak())
...
compare_data = [
    ...
    ['写真対応', '×', '×', '◎', '×', '<b>◎ 30 秒</b>'],
    ['弱点分析', '△', '△', '△', '△', '<b>◎ 日次自動</b>'],
    ['問題数', '5,000', '不明', '講師次第', '6,000', '<b>16,661</b>'],
    ['保護者可視化', '○', '○', '△', '×', '<b>◎</b>'],
    ['紹介特典', '×', '×', '×', '×', '<b>◎ ¥3,000 OFF</b>'],
]
...
# ============================================================
# CHAPTER 15: 全機能ガイド 実画面版 (18 機能・実スクショ)
# ============================================================
story.append(PageBreak())
story.append(Paragraph('CHAPTER 15', S['h1_chapter']))
story.append(Paragraph('全機能ガイド ─ 実画面で見る ai-juku', S['h1']))
story.append(GoldUnderline())
story.append(Spacer(1, 6 * mm))
story.append(Paragraph(
    'ここから先は <b>ai-juku 本番画面そのもの</b> です。すべてのスクリーンショットは '
    '<b>trillion-ai-juku.com</b> から取得した実画面で、生徒の個人情報は一切含まれていません。'
    '<b>18 の主要機能</b>を 1 機能 1 ページで詳細解説します。', S['body']))
story.append(Spacer(1, 4 * mm))
...
add_feature_page(story, 2, TOTAL,
    ...
    [
        '<b>本番想定難易度</b>: 16,661 問の問題プールから志望校別に AI が組み立て',
...
add_feature_page(story, 11, TOTAL,
    ...
    '集客',
    'お友達を紹介すると、紹介者は月額 ¥3,000 OFF・紹介された側は入塾金免除。'
    '双方が得する仕組みで、信頼できる仲間と一緒に成績アップ。',
    'referral_lp_1440x1600.png',
    [
        '<b>紹介者特典</b>: 月額 ¥14,500 → ¥11,500 (¥3,000 OFF 永久)',
...
add_feature_page(story, 14, TOTAL,
    ...
    [
        '<b>入塾金 ¥10,000 → 無料</b>: 14 日間無料体験で気に入ったら自動で入塾',
