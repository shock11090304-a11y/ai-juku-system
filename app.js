...
async function callClaude(systemPrompt, userMessage, options = {}) {
  ...
  const prev = inflightAbortControllers.get(kind);
  if (prev) { try { prev.abort(); } catch {} }
  const controller = new AbortController();
  inflightAbortControllers.set(kind, controller);
  ...
  if (backendOK) {
    try {
      ...
      if (res.ok) {
        ...
        if (inflightAbortControllers.get(kind) === controller) inflightAbortControllers.delete(kind);
...
async function sendChatMessage() {
  ...
  const systemPrompt = `あなたは優秀な学習塾のAIチューターです。生徒は${student.grade}の${student.name}さんで、志望校は${student.goal}です。
現在の科目は「${subject}」です。
...
- 回答は簡潔にしつつ、重要な概念はしっかり説明する
- 英語科目では、可能な限り志望校の過去問採点基準(語数・減点ポイント)に沿って解説
${/数学|物理|化学/.test(subject) ? `- 数式は LaTeX 記法で記述する（インライン \\( ... \\)、ディスプレイ \\[ ... \\]）。分数は \\dfrac、根号は \\sqrt、積分は \\int、総和は \\sum を用いる。全角記号（²・√・∫）やASCII混在（x^2, 1/2）は使わない
- 数学の論述では、使う定理・公式の根拠を明示し、場合分けは漏れなく列挙する。証明問題では「示すべき命題」を冒頭で宣言する
- 確率は独立性・復元/非復元を明記、整数は mod の法を明示、図形は座標・長さを文字で完全再現（画像依存禁止）
- 生徒の学年「${student.grade}」の履修範囲を逸脱した道具（数Ⅲの極限・複素平面・ベクトル外積など）は、必要最小限に留め、使う際は一言断る
...
async function generateProblems() {
  ...
  btn.textContent = '🎲 生成中...';
  // 旧結果を即座に消して体感を軽く
  out.innerHTML = `<div class="gen-loading">
    <p class="placeholder">🧪 AIがオリジナル問題を作成中... <span id="probElapsed">0秒</span></p>
    <p style="color:var(--text-dim);font-size:0.85rem;margin-top:0.5rem;">標準難易度: 約15-30秒、応用・難関: 約30-60秒</p>
  </div>`;
...
- 各問題は独立して解ける
- 解説は「なぜその答えになるか」の思考プロセスを含める
${/物理|化学|生物|理科/.test(subject) ? `- 【理科共通】単位は SI 単位系で統一（MKS: m, kg, s, A, K, mol, cd）。cgs との混在禁止。有効数字は基本2桁、入試準拠問題は3桁を原則とし明示
${/化学/.test(subject) ? `- 【化学の安全】劇物・毒物・爆発物（HF, KCN, NaN₃, ヒ素化合物, Hg塩, 濃硝酸の自己加熱等）の家庭での合成・実験手順は絶対に書かない。入試文脈での化学式・量論計算のみ許容
- 【化学の記法】官能基は R-COOH, R-NH₂, R-OH 等の統一記法。mol 計算では単位（mol, mol/L, g/mol）を明示` : ''}
...
${/国語（古文）|古文/.test(subject) ? `- 【古文専用】(a) 実在する古文作品（源氏/枕草子/徒然草/伊勢/大鏡等）の原文のみ使用。うろ覚えの一節を捏造せず、確実に知っている箇所のみ出題 (b) 助動詞設問は必ず①基本形②活用形③接続④意味⑤判別根拠 の5点を explanation で明示 (c)「る/らる」「す/さす」「む/むず」「なり」「らむ/けむ」の識別は全て判別フローを提示 (d) 敬語設問は「誰から誰への敬意か」を必ず問い、絶対敬語（奏す・啓す）と最高敬語（せ給ふ/させ給ふ）を区別` : ''}
${/国語（漢文）|漢文/.test(subject) ? `- 【漢文専用】返り点・書き下し・現代語訳の三点セット必須。再読文字・使役/受身句形は原文該当字を引用しながら解説` : ''}
${/数学/.test(subject) ? `- 【東大理系数学の採点基準】(a) 母数と場合分けの網羅性を冒頭で宣言 (b) 数え上げでは「どの対称性で割るか/割らないか」を一文で明示 (c) 幾何問題では使う定理（円周角の定理/方べき/余弦定理等）を本文中で引用してから適用 (d) 確率漸化式は状態遷移図または遷移行列を explanation で描画 (e) 最終値の等号成立条件・離散/連続の区別を結論で再確認` : ''}
${/数学/.test(subject) ? `
【数学専用の追加厳守事項】
...
上記の条件で、オリジナル練習問題を${count}問作成してください。各問題に解答・解説を必ず付けてください。`;
  ...
  // 難易度別に最適なモデル・設定を選ぶ（速度×品質のバランス）
  // 基礎/標準: Sonnet 4.6 で高速化、応用/難関: Opus 4.7 で品質維持
  const isHighStakes = difficulty === '応用' || difficulty === '難関';
  const problemModel = isHighStakes ? MODEL_PREMIUM : MODEL_STANDARD;
  // maxTokens を問題数でスケール。難関はthinkingで2500消費するため追加余裕を持たせる。
  // max_tokens = thinking + output のため、thinkingBudget + 実出力余裕 を確保。
  const thinkingBudget = isHighStakes ? 2500 : 0;
  const baseOutput = Number(count) * 550 + 1500;
  const dynamicMaxTokens = Math.min(8000, Math.max(2500, baseOutput + thinkingBudget));
  const useThinking = isHighStakes;

  let response;
  try {
    response = await callClaude(systemPrompt, userMsg, {
      kind: 'problems',
      model: problemModel,
    ...
    if (e.name === 'AbortError') {
      // 中断された場合は静かに終了（次のリクエストがUIを更新する）
      return;
    }
    out.innerHTML = `<p class="placeholder">⚠️ エラー: ${escapeHtml(e.message || String(e))}</p>`;
    btn.disabled = false;
  ...
  // Parse JSON response (複数の修復戦略を段階適用)
  let data;
  try {
    // 戦略0: trim + markdown codeblock 除去
    let clean = String(response || '').trim()
      .replace(/^```(?:json)?\s*\n?/, '')
      .replace(/\n?```\s*$/, '');
    const tryParse = (s) => { try { return JSON.parse(s); } catch { return null; } };
    // 戦略1: 先頭末尾の説明文を排除（最初の { と最後の } で括る）。
    // ただし truncation で末尾の } が実は途中の閉じかっこの場合、抽出すると
    // 2問目以降を丸ごと捨てるので、"抽出した結果が parse できる時のみ" 採用。
    ...
    if (firstBrace !== -1 && lastBrace > firstBrace) {
      ...
      if (testExtracted) {
        clean = extracted;  // 抽出成功かつ parse 可能なら採用
      } else if (firstBrace > 0) {
        // 前方の説明文だけ除去（後方は truncation で落とすと情報損失）
        clean = clean.substring(firstBrace);
      }
    }
    data = tryParse(clean);
    if (!data) {
      // 戦略2: 無効バックスラッシュ（LaTeX \(, \dfrac 等）を \\\\ に修復
      const fixEscape = clean.replace(/\\(?!["\\/bfnrtu])/g, '\\\\');
      data = tryParse(fixEscape);
      if (!data) {
        // 戦略3: 末尾カンマ除去（AI がよく入れる "foo": "bar", } など）
        const fixTrail = fixEscape.replace(/,(\s*[}\]])/g, '$1');
        data = tryParse(fixTrail);
      }
      if (!data) {
        // 戦略4: truncation 対策。文字列中のエスケープ・引用符を正しく追跡し、
        // 最後の未終端 " を閉じて、ネストの逆順で { や [ も閉じる。
        let t = clean;
        const stack = [];
        let inString = false;
        let escape = false;
        ...
        // 末尾カンマも再度除去（閉じかっこ追加で末尾カンマが露出するため）
        t = t.replace(/,(\s*[}\]])/g, '$1');
        data = tryParse(t);
      }
      if (!data) throw new Error('All repair strategies failed');
    }
  } catch (e) {
    console.warn('Problems JSON parse failed:', e, response);
    const snippet = escapeHtml(String(response || '').slice(0, 400));
    out.innerHTML = `<div class="placeholder" style="padding:1.5rem;line-height:1.7;">
      ...
      <details style="margin-top:1rem;font-size:0.8rem;color:var(--text-dim);">
        <summary style="cursor:pointer;">🔍 開発者用：生のAI応答（先頭400字）</summary>
        <pre style="background:rgba(0,0,0,0.3);padding:0.75rem;border-radius:6px;overflow:auto;white-space:pre-wrap;word-break:break-all;margin-top:0.5rem;">${snippet}</pre>
        <p style="margin-top:0.5rem;">Parse error: ${escapeHtml(e.message || String(e))}</p>
      </details>
    </div>`;
    actions.style.display = 'none';
    btn.disabled = false;
  ...
  window._lastProblemsContext = { subject, topic, count, difficulty, format, layout };
  window._lastProblemsData = data;
  window._lastProblemsMarkdown = convertDataToMarkdown(data);
}

function renderProblems(data, layout) {
