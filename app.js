...
【解説（explanation）の書き方 — 厳守】
解説は **苦手な生徒も得意な生徒も両方が伸びる教材** として書くこと。「答え合わせ」ではなく「独り学習の先生」を目指す。短すぎる解説は禁止。以下6セクションを原則全て含める（問題の性質で該当しないものは省略可）:

① 【この問題のポイント】(2〜4行)
   - 何を問うているか、どの単元・公式を使うか、なぜその方針を選ぶかを直感的に説明
   - 初見で詰まりやすい箇所を先に予告

② 【解き方ステップ】(段階的に、各ステップで「なぜそう変形するか」を1行添える)
   - 途中式を絶対に省略しない。「よって」「したがって」の間を埋める
   - 用語が出たら最小限の定義を添える（例:「mod 5 とは『5で割った余り』のこと」）
   - 1ステップ = 1文 + 1式 を目安。高校生が鉛筆でなぞれる粒度

③ 【答えの確認】(1〜2行)
   - 単位・次元・極限・特殊値の一つを代入して妥当性チェック
   - 「この答えで本当に元の式が成り立つか」を明示

④ 【よくあるミス】(2〜3個を箇条書き)
   - この単元で減点されやすい典型失点を具体例で示す
   - 「○○と間違えやすい」「ここで符号を落としがち」等
...
⑥ 【つまずいたら】(1行、苦手な生徒向け)
   - この問題の前に復習すべき単元・教材ページを具体的に
   - 例:「青チャート数Ⅰ 例題58が同パターン」「シス単 No.1120 の語法を確認」

文字数の目安: **基礎=200〜400字 / 標準=400〜700字 / 応用=700〜1200字 / 難関=1000〜2000字**。
全角記号の濫用は避け、見やすく改行する（\\n で改行、セクション見出しは【】で囲む）。
...
上記の条件で、オリジナル練習問題を${count}問作成してください。各問題に解答・解説を必ず付けてください。`;
  ...
  // 6セクション構造の詳細解説を要求しているため、1問あたり必要な出力量が増加。
  // 1問あたり ~1800 char 想定（難関で 2000 字）+ question/answer 分で ~400、
  // thinking も含めて余裕を持たせる（Anthropic max_tokens の上限は 16k）。
  const thinkingBudget = isHighStakes ? 3000 : 0;
  const baseOutput = Number(count) * 1200 + 1500;
  const dynamicMaxTokens = Math.min(16000, Math.max(3500, baseOutput + thinkingBudget));
