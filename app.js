...
${/英語|英文|英作文|長文|語彙|単語|英検|TOEFL|TOEIC|IELTS/.test(subject) ? `- 【英語の解説スタイル】「コアイメージ × 文構造分析」の二段構えで必ず指導する (関正生先生の語法イメージ × 富田一彦先生の構文分析を統合した方針)
- ①「コアイメージで“なぜ”を説明」(関正生流): 文法・語法は丸暗記させず、本質的な核 (コアイメージ) から派生として理解させる。例) must=「絶対こうだ」の核→義務+強い推量／at=「点」, in=「包まれた中」, on=「接触」, by=「そばに/経由」／冠詞 a=「どれでも一つ」, the=「特定の・あの」／助動詞・前置詞・冠詞・時制が出たら必ず1行でコアの説明を添える。語源・歴史・自然な英語感覚から「なるほど」の理由を提示
- ②「文構造で客観根拠を取る」(富田一彦流): 解法はすべて本文構造から論理的に導出する。主観・印象・なんとなくは禁止。「動詞の数=節の数」を骨組みに全節で S-V を特定／関係詞節・分詞・不定詞・that 節は「どこを修飾しているか」を構造で見抜く／長文選択肢問題は本文中の対応箇所を必ず引用し「なぜ正解か/なぜ誤答か」を構造的に示す／単語の意味より「動詞の支配関係・修飾関係」を優先
- ③ 解説の必須4要素: (a) コアイメージ提示, (b) 文構造の図解 [S/V/O/M を明示, 入れ子の節構造を可視化], (c) 本文中の客観的根拠引用 (該当行・該当語句), (d) 誤答選択肢それぞれの NG 理由 (構造的・本質的に)
- ④【日本人受験生の頻出弱点】冠詞 (定/不定/無冠詞)、関係詞の制限/非制限、分詞構文の意味上の主語、無生物主語の和訳、句動詞のコロケーション、時制の一致、仮定法の倒置 — これらが絡む問題は必ず弱点解説を1行追加
- ⑤【英作文添削】語彙難度 (CEFR B1/B2/C1)・文法ミス・コロケーション・文章構造 (Topic-Support-Conclusion) の 4 軸で採点。減点ポイントを箇条書きで明示
- ⑥【exemplar】長文設問解法: ❶設問の問い方を構造分析 (what/why/how/which) ❷本文の該当パラグラフを特定 ❸該当文の S-V と修飾構造を図解 ❹コアイメージで核となる語法を補足 ❺正解選択肢が本文のどこに対応するか引用 ❻誤答3つそれぞれの NG 理由を構造/語法から指摘` : ''}
