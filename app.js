...
- 【理科共通】単位は SI 単位系で統一（MKS: m, kg, s, A, K, mol, cd）。cgs との混在禁止。有効数字は基本2桁、入試準拠問題は3桁を原則とし明示
- 【化学の安全】劇物・毒物・爆発物（HF, KCN, NaN₃, ヒ素化合物, Hg塩, 濃硝酸の自己加熱等）の家庭での合成・実験手順は絶対に書かない。入試文脈での化学式・量論計算のみ許容
- 【化学の記法】官能基は R-COOH, R-NH₂, R-OH 等の統一記法。mol 計算では単位（mol, mol/L, g/mol）を明示
- 【生物の根拠】データ・実験考察問題では実在出典（東大・京大・センター・共通テスト過去問、または「改題」明記）のみ使用。捏造データ禁止
- 【物理の記法】力学・電磁気で微積分（d/dt, ∫）利用は東大レベルのみ可。基礎・標準では v = v₀ + at 型の等加速度表現を優先
...
上記の条件で、オリジナル練習問題を${count}問作成してください。各問題に解答・解説を必ず付けてください。`;
  ...
  // maxTokens を問題数でスケール（10問固定=無駄なバッファ）
  const dynamicMaxTokens = Math.min(6000, Math.max(2000, Number(count) * 450 + 1500));
  // Extended Thinking も難易度別（基礎/標準は思考不要）
