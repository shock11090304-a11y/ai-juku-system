...
--   6. 手書きストロークの形 (設計書 §5)・正規化座標だけを通す
--   7. 手書きの検査でキーの欠落を弾けていなかったのを直す
--   8. 教科を英語 4 種類から数学・国語・理科・社会へ広げる
--   9. 講師が PDF を上げられない穴の修正 (アップロードの RETURNING が読み取りポリシーで弾かれていた)
--   10. サブスク販売の契約表 (Stripe と生徒アカウントの対応・書くのは webhook だけ)
--   11. 動作確認用の中身 (仮定法の演習 1 冊 5 問 + 未公開の模試 1 冊)
--
--   ★ 11 (seed) は動作確認用。本番に入れるときは末尾の seed 部分を消して貼ること。
--
-- ■ 何度貼っても安全
...
-- ##############################################################################
-- ## 7. 手書きの検査でキーの欠落を弾けていなかったのを直す
-- ##    (supabase/migrations/20260814010000_english_learning_strokes_missing_keys.sql)
-- ##############################################################################
...
-- ##############################################################################
-- ## 9. 講師が PDF を上げられない穴の修正 (アップロードの RETURNING が読み取りポリシーで弾かれていた)
-- ##    (supabase/migrations/20260814030000_storage_teacher_read_fix.sql)
-- ##############################################################################
...
-- ##############################################################################
-- ## 10. サブスク販売の契約表 (Stripe と生徒アカウントの対応・書くのは webhook だけ)
-- ##    (supabase/migrations/20260816010000_subscriptions.sql)
-- ##############################################################################
