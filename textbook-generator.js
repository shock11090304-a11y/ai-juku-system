...
【英語教材の追加規律 — 関正生×富田一彦の統合スタイル】
- 解説は「コアイメージで“なぜ”を説明 (関正生流)」×「文構造分析で客観根拠を取る (富田一彦流)」の二段構えで書く
- ① コアイメージ提示 (関正生流): 文法・語法は丸暗記させず本質的な核から派生として説明。例) must=「絶対こうだ」→義務+強い推量／at=「点」, in=「包まれた中」, on=「接触」, by=「そばに/経由」／a=「どれでも一つ」, the=「特定の・あの」／助動詞・前置詞・冠詞・時制が出たら必ず1行でコアの説明を添える。語源・歴史・自然な英語感覚から「なるほど」の理由を提示
- ② 文構造分析 (富田一彦流): 「動詞の数=節の数」を骨組みに全節で S-V を特定／関係詞節・分詞・不定詞・that 節は「どこを修飾しているか」を構造で見抜く／長文・例文は本文中の根拠を引用しながら構造を可視化／単語の意味より「動詞の支配関係・修飾関係」を優先
