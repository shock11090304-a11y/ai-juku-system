...
async function generateTextbook() {
  ...
  const levelSpecMap = {
    ...
    '高校応用': {
      ...
      style: '精密な文構造分析を中心に据える。例題には類題 (早稲田 文 2018, 慶應 経 2020 等) を必ず添える',
    },
    '難関大受験': {
...
` : ''}${/英語|英文法|英作文|長文|語彙|単語|英検|TOEFL|TOEIC|IELTS/.test(subject) ? `
【英語教材の追加規律 — コアイメージ×文構造分析の二段構え】
- 解説は「コアイメージで“なぜ”を説明」×「文構造分析で客観根拠を取る」の二段構えで書く
- ① コアイメージ提示: 文法・語法は丸暗記させず本質的な核から派生として説明。例) must=「絶対こうだ」→義務+強い推量／at=「点」, in=「包まれた中」, on=「接触」, by=「そばに/経由」／a=「どれでも一つ」, the=「特定の・あの」／助動詞・前置詞・冠詞・時制が出たら必ず1行でコアの説明を添える。語源・歴史・自然な英語感覚から「なるほど」の理由を提示
- ② 文構造分析: 「動詞の数=節の数」を骨組みに全節で S-V を特定／関係詞節・分詞・不定詞・that 節は「どこを修飾しているか」を構造で見抜く／長文・例文は本文中の根拠を引用しながら構造を可視化／単語の意味より「動詞の支配関係・修飾関係」を優先
- ③ 例題・演習の解説は (a) コアイメージで本質説明 → (b) 文構造の図解 [S/V/O/M を明示] → (c) 客観的根拠の引用 → (d) 誤答 NG 理由 の順で構成
- ④ 「よくあるミス」セクション必須項目: 冠詞 (定/不定/無冠詞)、関係詞の制限/非制限、分詞構文の意味上の主語、無生物主語の和訳、句動詞のコロケーション、時制の一致、仮定法の倒置 — レベル「${level}」に応じて2件以上具体例で記す
- ⑤ 英作文セクションは語彙難度 (CEFR B1/B2/C1)・文法ミス・コロケーション・文章構造 (Topic-Support-Conclusion) の 4 軸で添削例を示す
` : ''}${notes ? `\n追加要件:\n${notes}` : ''}`;
