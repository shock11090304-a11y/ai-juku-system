    ...
    for (let attempt = 0; attempt < 2 && (!data || !data.selected); attempt++) {
      for (let bi = 0; bi < backends.length; bi++) {
        ...
        try {
          ...
          //    ログイン生徒の直近7日多様性除外を維持するため Bearer 送出。token無→匿名(除外なし)。
          const _eeBankTok = (window.AuthGuard && window.AuthGuard.getToken && window.AuthGuard.getToken()) || localStorage.getItem('ai_juku_session_token') || '';
          res = await fetch(url, {
            signal: ctrl.signal,
            cache: 'no-store',
            credentials: 'same-origin',
            headers: _eeBankTok ? { 'Authorization': 'Bearer ' + _eeBankTok } : {},
          });
          clearTimeout(timer);
          if (res && res.ok) {
