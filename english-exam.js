...
async function prefetchAutoGenerated(examId, sectionKey, eikenGrade) {
  try {
    ...
    console.log(`[exam] Loaded ${data.count} AI-generated questions for ${examId}/${storeKey}` + (studentId ? ` (student=${studentId})` : '') + (topicHint ? ` topic=${topicHint}` : ''));
    // セッション attempt push hook (app.js の sessionState を共有・関数があれば活用)
    try {
      if (Array.isArray(data.selected) && typeof window._pushSessionAttempt === 'function') {
        // bank ロード時点では正誤未確定。push は採点時 (submitAnswers/scoreLocally) で実施。
        // ここでは sessionState の問題リストにメタ追加だけ行う。
