...
async function prefetchAutoGenerated(examId, sectionKey, eikenGrade) {
  ...
  try {
    ...
    for (let bi = 0; bi < backends.length; bi++) {
      ...
      // 🛟 2026-05-16 timeout 修正:
      //   localhost backend は死んでる可能性高 → 3s で fast-fail → production fallback
      //   production / Vercel rewrite は cold start で 5-8s かかることもある → 12s 寛容に
      //   (旧 3s timeout は production cold start で abort → fallback に落ちる致命傷)
