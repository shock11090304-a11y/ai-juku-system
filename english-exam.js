...
async function prefetchAutoGenerated(examId, sectionKey, eikenGrade) {
  ...
  try {
    ...
    for (let bi = 0; bi < backends.length; bi++) {
      ...
      // 3 秒 timeout で hung backend に長時間ブロックされないように (3 視点 review 反映)
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 3000);
      // 🛡 production fallback 時は dev_preview=1 を付けて Railway access log を分離
      //    (本番 student analytics の汚染を防ぐ / Security review 反映)
    ...
    if (!res) {
      console.warn('[exam] pool fetch failed on all backends:', lastErr);
      return null;
    }
    if (!res.ok) return null;
    const data = await res.json();
    if (!data || !data.selected) {
      // pool empty: 統計に残す (UI badge で「pool 不在」を判定)
      console.log(`[exam] pool miss for ${examId}/${sectionKey}/${eikenGrade || '-'} (count=${(data && data.count) || 0})`);
      return data || null;
    }
...
function demoQuestions(exam, section, qCount, topic) {
  ...
  let warningMsg;
  if (isProduction) {
    warningMsg = `🔄 ${exam.name} ${section.name} の問題を読み込んでいます。少し待ってから「実行」を再度押すか、ページを再読み込みしてください (問題は順次表示されます)。`;
  } else if (isLocalPreview) {
    warningMsg = `⚠️ ローカル preview 環境では問題プールに接続できないことがあります。本番 URL (trillion-ai-juku.com) で問題プールから即時取り出されます。`;
  } else {
    warningMsg = `🔄 ${exam.name} ${section.name} の問題を読み込んでいます。通信が不安定なようです。少し待って再度お試しください。`;
  }
  return {
    passage: '',
    audio_script: '',
