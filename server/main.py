...
【解説の指導哲学 — 関正生×富田一彦の統合スタイル (必ず厳守)】
解説は「コアイメージで“なぜ”を説明 (関正生流)」×「文構造分析で客観根拠を取る (富田一彦流)」の二段構えで書く:
① コアイメージ提示: 文法・語法は丸暗記でなく本質的な核 (コアイメージ) から派生として説明する。例) must=「絶対こうだ」→義務+強い推量／at=「点」, in=「包まれた中」, on=「接触」／the=「特定の・あの」／時制・助動詞・前置詞・冠詞が出たら必ず1行でコアの説明を添える
② 文構造分析: 解法は本文構造から論理的に導出する。「動詞の数=節の数」を骨組みに S-V を全節で特定／関係詞節・分詞・不定詞・that 節は「どこを修飾しているか」を構造で見抜く／長文選択肢問題は本文中の対応箇所を引用し「なぜ正解か/なぜ誤答か」を構造的に示す
③ 解説の必須要素 (3行以上、できれば5行): (a)コアイメージで本質説明 (b)文構造の図解 [S/V/O 明示] (c)本文中の客観的根拠引用 (d)誤答選択肢それぞれの NG 理由 (構造的・本質的に) (e)関連語彙/文法の派生
...
【解説の指導哲学 — 関正生×富田一彦の統合スタイル (必ず厳守)】
解説は「コアイメージで“なぜ”を説明 (関正生流)」×「文構造分析で客観根拠を取る (富田一彦流)」の二段構え:
① コアイメージ提示: 文法・語法は丸暗記でなく本質的な核 (コアイメージ) から派生として説明する。例) must=「絶対こうだ」／at=「点」, in=「中」, on=「接触」／the=「特定の・あの」／助動詞・前置詞・冠詞・時制が出たら必ず1行でコアの説明を添える
② 文構造分析: 解法は本文構造から論理的に導出する。「動詞の数=節の数」で S-V を全節特定／関係詞・分詞・不定詞・that 節は修飾関係を構造で見抜く／選択肢問題は本文中の対応箇所を引用し構造的に正解/誤答を示す
③ 必須要素: (a)コアイメージ提示 (b)文構造の図解 [S/V/O 明示] (c)本文中の根拠引用 (d)誤答3つそれぞれの NG 理由
