#!/bin/bash
# 🆓 Claude Max 経由生成の教材 JSON を一括 import するヘルパー
# 使い方:
#   cd ~/ai-juku-system
#   ./server/import_seed_textbooks.sh server/seed_textbooks_batch01.json
#   ./server/import_seed_textbooks.sh server/seed_textbooks_batch*.json   # 全バッチ一括
#
...
for FILE in "$@"; do
    ...
    echo "📥 importing: $FILE"
    RESPONSE=$(curl -sS -m 60 -X POST "$API_URL/api/admin/textbooks/import" \
    ...
    if [ "$FAILED" != "0" ]; then
        echo "  ↳ failed_details:"
        echo "$RESPONSE" | python3 -c "import sys,json; [print('     ',d) for d in json.load(sys.stdin).get('failed_details',[])]" 2>/dev/null || true
    fi
...
echo ""
echo "✅ 完了: 計 $TOTAL_INSERTED 教材 import (失敗 $TOTAL_FAILED 教材)"
